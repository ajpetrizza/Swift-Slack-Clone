//
//  MessageService.swift
//  Smack
//
//  Created by Anthony Petrizza on 1/17/18.
//  Copyright © 2018 Anthony Petrizza. All rights reserved.
//

import Foundation
import Alamofire
import SwiftyJSON

class MessageService {
    
    static let instance = MessageService()
    
    // array of channel objects
    var channels = [Channel]()
    var messages = [Message]()
    var selectedChannel: Channel?
    
    //GET all the channels
    func findAllChannels(completion: @escaping CompletionHandler) {
        Alamofire.request(URL_GET_CHANNELS, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: BEARER_HEADER).responseJSON { (response) in
            
            if response.result.error == nil {
                guard let data = response.data else {return}
                if let json = try? JSON(data: data).array {
                    // loop through the array of json objects and grab each value we need
                    for item in json! {
                        let name = item["name"].stringValue
                        let channelDescription = item["description"].stringValue
                        let id = item["_id"].stringValue

                        // create new channel object with our data and add it to our array
                        let channel = Channel(channelTitle: name, channelDescription: channelDescription, id: id)
                        self.channels.append(channel)
                    }
                    // sends out notification that the channels have loaded
                    NotificationCenter.default.post(name: NOTIF_CHANNELS_LOADED, object: nil)
                    completion(true)
                }

            } else {
                completion(false)
                debugPrint("error in FINDALLCHANNELS")
                debugPrint(response.result.error as Any)
            }
        }
    }
    
    // Grabbing all messages
    func findAllMessagesForChannel(channelId: String, completion: @escaping CompletionHandler) {
        
        Alamofire.request("\(URL_GET_MESSAGES)\(channelId)", method: .get, parameters: nil, encoding: JSONEncoding.default, headers: BEARER_HEADER).responseJSON { (response) in
            if response.result.error == nil {
                // first clear old messages
                self.clearMessages()
                guard let data = response.data else {return}
                //force unwrap because we know data is there
                if let json = try! JSON(data: data).array {
                    for item in json {
                        let messageBody = item["messageBody"].stringValue
                        let channelId = item["channelId"].stringValue
                        let id = item["_id"].stringValue
                        let userName = item["userName"].stringValue
                        let userAvatar = item["userAvatar"].stringValue
                        let userAvatarColor = item["userAvatarColor"].stringValue
                        let timeStamp = item["timeStamp"].stringValue
                        
                        // create new message with all of the info
                        let message = Message(message: messageBody, userName: userName, channelId: channelId, userAvatar: userAvatar,
                                              userAvatarColor: userAvatarColor, id: id, timeStamp: timeStamp)
                        // append message to messages
                        self.messages.append(message)
                    }
//                    print(self.messages)
                    completion(true)
                }
            } else {
                debugPrint(response.result.error as Any)
                debugPrint("Error result in FINDALLMESSAGESFORCHANNEL")
                completion(false)
            }
        }
        
    }
    
    func clearMessages() {
        messages.removeAll()
    }
    
    func clearChannels() {
        channels.removeAll()
    }
}







